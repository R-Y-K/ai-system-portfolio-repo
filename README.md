...
In case it was not clear, this is not Large Language Model (LLM) AI, but a Logic-Based AI. No ChatGPT or Claude self-aware AI here folks.
...
There are many, many components that are needed for the AI to be able to function, which I will not be explaining or sharing here, because it would way too verbose, and I want to focus on the AI system, rather than the ecosystem which makes running the AI possible.

This AI is designed in a modular way, that this system is not inherently dependant on those components themselves, but for the outputs that the assigned components should produce. What this means is that if you want to use this AI, and want to write the your own custom systems that produce the required outputs for the required inputs that the AI requires, it should be possible to do so.
...
For now I'll explain the general twist of how the AI works, but I'll explain each layer in detail later when we go through them one by one. The AI system that I've created relies heavily on rules, or as I like to call them "conditions", which decide when AI is going to do something. Creating and managing conditions when to run or not run behaviours or actions is at the very heart of this system. 

Each layer is an individual finite state machine responsible for running different BehaviourControllers for the AI decision making. They responsible for running individual states inside controllers, in order to fulfill a purpose for the NPC in question. Each layer controls the controller of the layer below it. This creates a execution flow in which each layer runs different controllers for the states below them, based on the conditions in the controller controlling the layers.

Every state machine runs the same way, running one state at a time with the priority being the one on top of the list, exception being the StepBehaviourController in Step/Action Layer, in which we need to run and manage actions in parallel, in order to make the AI able to do multiple actions at the same time.
...
MasterController holds all information and components that are required for the AI to function. States use it container to access all required information, that are required by AI the to run. It is responsible for deciding which BehaviourControllers to use. It uses that information to decide what kind of "role" it should take depending on the conditions assigned to the AI.
...
Tactics layer is the "HOW" of the AI

BehaviourController in the Tactics Layer assumes that the MasterController in the Role Layer has decided on a role and now needs to run a specific set of actions in order to fulfill that role.

It holds GoalBehaviourControllers, which it uses for setting up a list of goals for the BehaviourController to achieve, depending on the conditions that have been set up in the GoalBehaviourController.
...
AttackNearestEnemy GoalBehaviourController has a set of actions for the TryToAttackNearestEnemy StepBehaviourController to run in order to reach the desired out come of the goal.
...
These set of instructions are run infinitely until the conditions for enabling the actions or behaviours changes.
